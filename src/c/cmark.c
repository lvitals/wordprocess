...
static int cmark_parse_cb(lua_State* L)
{
    ...
    cmark_node** ud = (cmark_node**)lua_newuserdata(L, sizeof(cmark_node*));
    *ud = document;
...
{
    cmark_node** ud = (cmark_node**)lua_touserdata(L, 1);
    cmark_node_free(*ud);
    return 0;
}
...
static int cmark_iterate_cb(lua_State* L)
{
    ...
    cmark_iter** ud = (cmark_iter**)lua_newuserdata(L, sizeof(cmark_iter*));
    *ud = iter;
...
{
    cmark_iter** ud = (cmark_iter**)lua_touserdata(L, 1);
    cmark_iter_free(*ud);
    return 0;
}
