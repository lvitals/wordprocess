...
CONFIGDIR = HOME .. "/.wordprocess"
local configfile = CONFIGDIR.."/startup.lua"

local LAST_RECENT_FILE= "last_recent_file"
...
function WordProcessor(filename)
    ...
    do
        ...
        movefile(HOME.."/.wordprocess.settings", CONFIGDIR.."/settings.dat")
        movefile(HOME.."/.wordprocess.lua", CONFIGDIR.."/startup.lua")
    end
    ...
    if filename then
        if filename == LAST_RECENT_FILE then
            local r = (GlobalSettings.recents or {})
            filename = r[1]
    ...
    local masterkeymap= {
		...
		-- A terminal normally cannot distinguish Tab from Ctrl-I, so the plain
		-- code must keep Tab's editing meaning. Graphical backends can report an
		-- explicit Ctrl-I and continue to support the italic shortcut.
		["\t"] = GroupCallback{ Cmd.Checkpoint, Cmd.TypeWhileSelected,
			Cmd.InsertTab },
    ...
    NonmodalMessage("Welcome to WordProcess! Press ESC to show the menu.")
    while true do
        local xpcall = (xpcall)
        ...
        if not f then
            ...
            ModalMessage("Internal error!",
                "Something went wrong inside WordProcess! I'll try and "..
...
function Main(...)
    ...
    do
        local function do_help()
            PrintErr("WordProcess version ", VERSION, " © 2026 Leandro V. Catarin\n")
            if DEBUG then
                PrintErr("(This version has been compiled with debugging enabled.)\n")
            end

            PrintErr([[
Syntax: wp [<options...>] [<filename>]
Options:
...
Only one filename may be specified, which is the name of a WordProcess
...
extension. To specify a document name, use :name as a suffix. e.g.:
    ...
    wp --convert filename.wp:"Chapter 1" chapter1.odt
