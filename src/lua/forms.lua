...
local WrappedLabel= Form.Widget {
	draw = function(self)
...
local Toggle= Form.Widget {
	...
	draw = function(self)
		...
		local selectorwidth = math.min(self.realwidth,
			math.max(12, GetStringWidth(s)))
		local selectorx = self.realx2 - selectorwidth + 1
		Write(self.realx1, self.realy1,
			GetBoundedString(self.label, math.max(selectorx - self.realx1 - 1, 0)))

		SetBright()
		Write(selectorx, self.realy1, GetBoundedString(s, selectorwidth))
		SetNormal()

		if self.focus then
			GotoXY(math.min(selectorx + 2, self.realx2), self.realy1)
		end
	end,
...
local function findaction(table, focus, key)
...
local function findmouseaction(form, m)
	local x = m.x
	local y = m.y
	for i, widget in ipairs(form.widgets) do
...
function Form.Run(form, redraw, helptext)
	local function redraw_form()
		...
		for _, widget in ipairs(form.widgets) do
			if widget.init then
				widget:init()
			end
		end
