...
function CreateMenu(n, m, replaces)
	local w = n:len()
	local menu= {
		label = "",
		maxwidth = 0,
...
local function E(id, mk, label, ak,
		...)
	return {
		id = id,
		mk = mk,
...
local function M(id, mk, label, ak,
	menu)
	return {
		id = id,
		mk = mk,
...
local FileMenu = CreateMenu("File",
{
	...
	separator,
	E("Fabout",     "Z", "About WordProcess...",      nil,         Cmd.AboutWordProcess),
...
local NavigationMenu = CreateMenu("Navigation",
{
	...
	E("ZSA",    nil, "Select all",                   "^A",         Cmd.SelectAll),
	...
	E("ZITAB",  nil, "Insert tabulation",            "^T",       cp, Cmd.Checkpoint, Cmd.TypeWhileSelected, Cmd.InsertTab),
...
function MenuTree.runmenu(self, x, y, menu)
	local n = 1
	local top = 1
